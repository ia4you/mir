...
// Orden temporal: Groq primero, Gemini como fallback, mientras el proyecto
// de Gemini tiene la cuota del free tier a 0 (ver conversación de despliegue).
// Cuando Gemini tenga cuota disponible, revertir a Gemini primero / Groq
// fallback, que es el orden preferido según la spec original.
//
// Una única llamada de IA por test. Ante cualquier fallo del proveedor
// principal (cuota, timeout, 5xx, o cualquier otro error de red/API) se hace
// fallback al secundario con el mismo prompt. Si ambos fallan, la pantalla
// de resultados debe seguir funcionando solo con los datos calculados por
// código, así que aquí nunca se relanza el error: se devuelve texto null.
export async function generarAnalisisNarrativo(datos) {
  ...
  try {
    ...
    console.error("resumen-test: fallo Groq, probando Gemini:", errGroq.message);
    try {
      const texto = await generarConGemini({ systemPrompt: SYSTEM_PROMPT, userPrompt });
      return { texto, proveedor: "gemini" };
    } catch (errGemini) {
      console.error("resumen-test: fallo Gemini, sin análisis narrativo:", errGemini.message);
